...
def try_to_plop(mouse_position: Tuple[int, int]) -> bool:
    ...
    if state.is_cursor_in_buildable_space():
        ...
        if tower_definition:
            logging.debug(f'Tower definition {tower_definition.tower_name} has been selected')
    ...
    def calculate_rays(self, surface, left, top):
        ...
        for tower in self.plops.values():
            if tower.is_reloaded():
                ...
                tower_can_see_invisible = tower.ploppable.get_tower_stats()['special'] == 'sees invisible'
                tower_slows_enemy = tower.ploppable.get_tower_stats()['special'] == 'slows enemy'
                ...
                for critter in critters:
                    visibility_test = (not critter.is_invisible()) or tower_can_see_invisible
                    ...
                    if visibility_test and slow_test:
                        ...
                        if line_length <= tower_range:
                            pygame.draw.aaline(surface, RAY_COLOR, tower_pos, critter_pos, 5)
